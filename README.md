...
- **Tabela de relação:** `tickets_with_clockify` possui uma linha por ticket
  ...
  `clockify_actual_hours` e `jira_logged_hours` para auditoria e define
  `spent_hours` como o maior dos dois valores. `spent_source` identifica a
  fonte escolhida.
...
- **Campos ausentes:** `jira_logged_hours` vazio não invalida o ticket; o
  Clockify é usado como fallback. Estimativa ausente não entra na média de
  estimativa, variação ou razão. O tempo gasto válido continua podendo entrar
  na média de tempo real.
- **Proteções:** horas negativas, infinitas ou não numéricas são descartadas
  durante a normalização. A tabela final só aceita lançamentos Clockify
  positivos e valida novamente a fonte escolhida e a variação antes de gerar o
  JSON.
...
regra de maior valor entre Jira e Clockify. A estimativa, `avg_delta_hours`,
`actual_to_estimate_ratio` e `coverage_pct` ajudam a explicar o movimento sem
substituir o indicador principal.
